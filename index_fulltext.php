...
elseif ($msPage != '') {
   if ($msPage > 78 && $msPage < 88) $error = '<p>The manuscript pagination jumps from <a href="index_fulltext.php?ms=78">78</a> to <a href="index_fulltext.php?ms=88">88</a> (even though there appears to be no textual lacuna). Click either number to go to that page.</p>';
   elseif ($msPage > 249) $error = '<p>The manuscript ends at p. <a href="index_fulltext.php?ms=249">249</a>.</p>';
   else {
...
if ($keilVol == 2 && $keilPage > 597) $error = '<p><i>GL</i> 2 ends at p. <a href="index_fulltext.php?kV=2&kP=597">597</a>.</p>';
elseif ($msCutOff && ($keilVol == 3 && $keilPage > 147)) $error = '<p>This edition displays only the text of Priscian to <i>GL</i> III, p. <a href="index_fulltext.php?kV=3&kP=147">147</a>, where the St Gall manuscript ends (at <i>naturaliter</i>, line 18).</p>';
...
<form action="index_fulltext.php" method="get">
<p> 
...
//         print '</p>';
         ...
         while ($rowP < mysql_num_rows($resultsP)) {
            // line of Priscian
            print '<tr>';
            print '<td class="priscian" nowrap="nowrap"><a class="tooltip" title="Vol., page and line of Hertz\'s edition in Keil, <i>Grammatici Latini</i>. <br/>Click to go to context in Priscian. " href="index_fulltext.php?kV=' . mysql_result($resultsP, $rowP, 'keil_vol') . '&amp;kP=' . mysql_result($resultsP, $rowP, 'keil_page') . '&amp;kL=' . mysql_result($resultsP, $rowP, 'keil_line') . '#hi">' . volConvert(mysql_result($resultsP, $rowP, 'keil_vol')) . ' ' . mysql_result($resultsP, $rowP, 'keil_page') . ',' . mysql_result($resultsP, $rowP, 'keil_line') . '</td>';
...
elseif ($keilVol != '' && $keilPage != '') {
   ...
   if (mysql_num_rows($resultsP) == 0) {
      ...
      print '<p>The page in Keil may comprise variant apparatus only. Try pages 
         <a href="index_fulltext.php?kV=' . $keilVol . '&amp;kP=' . ($keilPage - 1) . '#hi">' . ($keilPage - 1) . '</a> or 
         <a href="index_fulltext.php?kV=' . $keilVol . '&amp;kP=' . ($keilPage + 1) . '#hi">' . ($keilPage + 1) . '</a>?</p>';
   }
   else {
      // heading      
      ...
      if ($prevPage > 0) print '<a class="button" href="index_fulltext.php?kV=' . $prevVol . '&amp;kP=' . $prevPage . '#end">previous page</a>';
//      if ($prevPage > 0 && $nextPage > 0) print ' | ';
      if ($nextPage > 0) print '<a class="button" href="index_fulltext.php?kV=' . $nextVol . '&amp;kP=' . $nextPage . '">next page</a>';
      print '</p>';
