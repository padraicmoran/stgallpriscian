...
function initVars($varName, $default) {
   ...
   elseif (isset($_GET[$varName])) return $_GET[$varName];
   else return $default;
}
...
function writeRadio($name, $value, $label, $testValue) {
   print '<input type="radio" name="' . $name . '" id="' . $name . '_' . $value . '" value="' . $value . '"';
...
function writeCheckbox($name, $value, $label, $testValue) {
   print '<input type="checkbox" name="' . $name . '" id="' . $name . '" value="' . $value . '"';
   if ($value == $testValue) print ' checked="checked"';
...
function changeKey($key1, $value1, $key2, $value2) {
   ...
   foreach($_GET As $getKey => $getValue) {
      if ($tmp != '') $tmp .= '&amp;';
      if ($getKey == $key1) {
         $tmp .= $getKey . '=' . urlencode($value1);
         $found1 = true;
      } 
      elseif ($getKey == $key2) {
         $tmp .= $getKey . '=' . urlencode($value2);
         $found2 = true;
      } 
      else $tmp .= $getKey . '=' . urlencode($getValue);
   }
   if (! $found1 && $key1 != '') { 
      if ($tmp != '') $tmp .= '&amp;';
      $tmp .= $key1 . '=' . urlencode($value1);
   }
   if (! $found2 && $key2 != '') { 
      if ($tmp != '') $tmp .= '&amp;';
      $tmp .= $key2 . '=' . urlencode($value2);
   }
   return $tmp;      
}
